...
import 'string.dart';
import 'package:stop_complaint/login.dart';
import 'package:stop_complaint/Model/order_model.dart';
import 'package:stop_complaint/notification_lIst.dart';
import 'package:stop_complaint/order_detail.dart';
import 'package:stop_complaint/privacy_policy.dart';
import 'package:stop_complaint/wallet_history.dart';
import 'package:stop_complaint/delivery.dart';
...
class StateHome extends State<Home1> with TickerProviderStateMixin {
  int curDrwSel = 0;
  ...
  getData2() async {
    ...
    if(ids==null)
    {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(quantity.toString()),));
    ...
    return Scaffold(
      ...
      body: _isNetworkAvail
          ? _isLoading
            ...
            context,
            MaterialPageRoute(
                builder: (context) => OrderDetail(orderid:orderList![index]['OrderId'],uid:orderList![index]['Uid'],
                  disprice:orderList![index]['Disprice'],prdtprice:orderList![index]['Prdtprice'],grandtotal:orderList![index]['Grandtotal'],
                  prdtcode:orderList![index]['Prdtcode'],quantity:orderList![index]['Quantity'],transactionid:orderList![index]['Transaction_id'],prdtname:orderList![index]['Prdtname'],
                  sphone:orderList![index]['Sphone'],saphone:orderList![index]['Saphone'],saddress:orderList![index]['Saddress'],
                  sname:orderList![index]['Sname'],slocation:orderList![index]['Slocation'],paymode:orderList![index]['paymode'],
                  pincode:orderList![index]['Spincode']
  ...
  Future<Null> getOrder() async {
    ...
    if (_isNetworkAvail) {
      ...
      try {
        ...
        Response response =await http.post(Uri.parse('https://meatbite.in/delivery/soldproduct1.php?ID='+pref.getString('Eroute').toString()),body: parameter, headers: headers)
...
// Fluttertoast.showToast(msg: pref.getString('Eroute').toString());
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(pref.getString('Eroute').toString()),));
        Response response =
